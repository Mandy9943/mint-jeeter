...
pub trait Operations: storage::Storage {
    ...
    fn mint(&self, amount_of_tokens: u32) {
        let payment_amount = self.call_value().single_esdt();
        ...
        let single_payment_amount = payment_amount.clone().amount / amount_of_tokens;

        let price_tag = self.selling_price().get();
        require!(
            single_payment_amount == price_tag.amount,
            "Invalid amount as payment"
        );
        require!(
            payment_amount.token_identifier == price_tag.token_identifier,
            "Invalid token as payment"
        );
    ...
    fn mint_single_nft(
        ...
        if payment_amount > 0 {
            ...
            self.send().direct_esdt(
                &owner,
                &price_tag.token_identifier,
                price_tag.token_nonce,
                &payment_amount,
            );
        }
